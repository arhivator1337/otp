	...
	**/
	function error($code,$text='',array $trace=NULL,$level=0) {
		...
		if ($highlight=!$this->hive['CLI'] && !$this->hive['AJAX'] &&
			$this->hive['HIGHLIGHT'] && is_file($css= __DIR__ . 'base.php/' .self::CSS))
...
class View extends Prefab {
	...
	**/
	...
	public function _($dict, $param1 = false, $param2 = false, $param3 = false) {
		$app = \Base::instance();

		if($param1)
			return ucfirst($app->format($app->get('' . $dict), $param1));
		else
			return $app->get('' . $dict);
	}
